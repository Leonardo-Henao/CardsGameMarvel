...
public class CommandHandle {
    ...
    @Bean
    public RouterFunction<ServerResponse> cambiarCarta(CambiarCartaUseCase useCase) {
        return route(
                POST("/juego/cambiar_carta").and(accept(MediaType.APPLICATION_JSON)),
                request -> useCase.andThen(integrationHandle)
                        .apply(request.bodyToMono(CambiarCartaCommand.class))
                        .then(ServerResponse.ok().build())
                        .onErrorResume(errorHandler::badRequest)
        );
    }
